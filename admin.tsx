Aquí tienes la solución exacta. El problema es que en el archivo `src/routes/admin.tsx` había varios errores combinados:

1. El `useEffect` de verificación de sesión estaba descomentado, pero la variable `ready` tenía un **error de sintaxis de JSX** (`useState(false;` en lugar de `useState(false);` sin cerrar el paréntesis).
2. El archivo estaba **incompleto / cortado** al final en la sección de `SettingsPanel`.

A continuación tienes el archivo **`src/routes/admin.tsx`** completamente corregido, sintácticamente válido y listo para reemplazar:
...
/* ─────────────────── AUXILIARY PANELS STUBS ─────────────────── */
function OrdersPanel() { return <div className="p-6 bg-card rounded-2xl shadow">Panel de Pedidos</div>; }
function BannersPanel() { return <div className="p-6 bg-card rounded-2xl shadow">Panel de Banners</div>; }
function TestimonialsAdminPanel() { return <div className="p-6 bg-card rounded-2xl shadow">Panel de Testimonios</div>; }
function CategoryImagesPanel({ value, onChange }: { value: any; onChange: (v: any) => void }) { return null; }
function AssetUploader({ title, hint, currentUrl, uploading, onFile, onUrlChange, previewClass }: any) {
  return (
    <div>
      <p className="font-semibold text-sm mb-1">{title}</p>
      <p className="text-xs text-muted-foreground mb-2">{hint}</p>
      {currentUrl && <img src={currentUrl} alt={title} className={`mb-2 rounded-xl ${previewClass}`} />}
      <input type="file" onChange={(e) => e.target.files?.[0] && onFile(e.target.files[0])} className="text-xs" />
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div className="rounded-2xl bg-card p-6 shadow-[var(--shadow-product)]">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <AssetUploader
            ...
            onFile={(f: File) => handleAssetUpload("logo", f)}
            onUrlChange={(v: string) => set("logo_url", v)}
          ...
          <AssetUploader
            ...
            onFile={(f: File) => handleAssetUpload("hero", f)}
            onUrlChange={(v: string) => set("hero_image_url", v)}
      ...
      <div className="rounded-2xl bg-card p-6 shadow-[var(--shadow-product)]">
        ...
        <div className="mt-6 flex items-center justify-between">
          {msg && <span className="text-sm font-bold text-primary">{msg}</span>}
          <button
            disabled={mut.isPending}
            onClick={() => mut.mutate(s)}
            className="ml-auto rounded-full bg-primary px-6 py-2 text-sm font-bold uppercase text-primary-foreground disabled:opacity-60"
          >
